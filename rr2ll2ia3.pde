...
public class rr2ll2ia3 extends PApplet {
...
float reproductionThreshold = (int) random(0.7f, 7);
...
class Cell {
    ...
    public void interact(Cell other) {
        ...
        if (distance < interactionRadius) {
            ...
            int geneChange = PApplet.parseInt(influence * (other.gene - this.gene) * 0.1f);
    ...
    public void reproduce(Cell other) {
    ...
    public int mutateGene(int gene) {
        ...
        if (mutationRoll < 0.26f) {
            mutationAmount = (random(1) < 0.5f) ? -1 : 1;
        } else if (mutationRoll < 0.9f) {
            mutationAmount = (random(1) < 0.5f) ? -2 : 2;
        } else {
            mutationAmount = (random(1) < 0.5f) ? -3 : 3;
        }
...
public void setup() {
    /* size commented out by preprocessor */; // siğ  fullScreen(P2D);
...
public void randomizeSimulation() {
    ...
    for (int i = 0; i < UNIVERSE_SIZE; i++) {
        ...
        cells.add(new Cell(random(width), random(height), PApplet.parseInt(random(GENE_MIN, GENE_MAX + 1)), initialMutationRate));
    }
...
public void setNewRefreshInterval() {
...
public void drawAxisLines() {
  ...
  for (int x = 0; x < axisLines.width; x++) {
    for (int y = 0; y < axisLines.height; y++) {
      ...
      if (
        (y >= axisLines.height / 2 - 0.4f && y <= axisLines.height / 2 && x >= dynamicGapLeft && x <= axisLines.width - dynamicGapRight ) ||
        (x >= axisLines.width / 2 - 0.4f && x <= axisLines.width / 2 && y >= fixedGapSize && y <= axisLines.height - fixedGapSize)
      ){
...
public void createNewCell() {
    float initialMutationRate = baseMutationRate + random(-mutationRateRange, mutationRateRange);
    cells.add(new Cell(mouseX, mouseY, PApplet.parseInt(random(GENE_MIN, GENE_MAX + 1)), initialMutationRate));
...
public void killCellsNearMouse() {
...
public int getInvertedColorArea(int x, int y) {
    int sumRed = 0;
    int sumGreen = 0;
    int sumBlue = 0;
    int sampleCount = 0;
...
public int getInvertedColor(int x, int y) {
...
public void updateCursor() {
    ...
    if (hoveringCell) {
        ...
        for (int x = 0; x < cellDiameter; x++) {
            for (int y = 0; y < cellDiameter; y++) {
                ...
                if (d <= cellDiameter / 5 && d > cellDiameter/ 5 - 0.9f) { //Only if d <= celldiameter and d in ring of stroke
  ...
  public void settings() { size(1680/3, 1020/3);
smooth(); }

  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "rr2ll2ia3" };
    if (passedArgs != null) {
